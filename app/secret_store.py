...
SECRET_CATALOG: list[dict] = [
    ...
    dict(key="go_model", env="GO_MODEL",
         label="مدل OpenCode", group="هوش مصنوعی", sensitive=False),
    dict(key="gemini_keys_path", env="GEMINI_KEYS_PATH",
         label="مسیر فایل کلیدهای Gemini", group="هوش مصنوعی", sensitive=False),
    dict(key="deepseek_api_key", env="DEEPSEEK_API_KEY",
         label="کلید DeepSeek", group="هوش مصنوعی", sensitive=True),
    dict(key="avalai_api_key", env="AVALAI_API_KEY",
         label="کلید Avalai", group="هوش مصنوعی", sensitive=True),
    dict(key="llm_order", env="LLM_ORDER",
         label="ترتیب ارائه‌دهنده‌های LLM", group="هوش مصنوعی", sensitive=False),
    # پیامک (OTP)
